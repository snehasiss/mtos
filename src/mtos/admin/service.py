...
class AdminService:
    def __init__(self, project_root, data_dir, *, runner=None):
    ...
    def _execute(self, operation, payload):
        ...
        if operation == "backup":
            ...
            return self._backup(payload.get("remote"))
        if operation == "restore":
            self._require_stopped()
            return self._restore(payload.get("remote"))
        if operation == "update":
            self._require_stopped()
            return self._update(payload.get("ref"))
    ...
    @staticmethod
    def _remote(value):
        match = REMOTE.fullmatch(value or "")
        if not match:
    ...
    def _update(self, ref):
        if not GIT_REF.fullmatch(ref or ""):
            raise ValueError("Invalid Git branch or tag")
        ...
        self._run(["git", "checkout", ref], cwd=self.project_root)
        branch = self._run(["git", "branch", "--show-current"], cwd=self.project_root).stdout.strip()
        if branch:
            self._run(["git", "pull", "--ff-only", "origin", branch], cwd=self.project_root)
        revision = self._run(["git", "rev-parse", "--short", "HEAD"], cwd=self.project_root).stdout.strip()
        return f"Checked out {ref} at {revision}; no commit created"
