...
def test_reverse_dependency_blocks_retirement(roster):
    ...
    with pytest.raises(ValueError, match="requires"):
        roster.save(
            {
                "revision": 1,
                "lifecycle": {"status": "retired", "retired_on": "2026-09-13"},
            },
            asset_id="L001",
        )
...
def test_backup_restores_database_and_media_and_detects_corruption(roster, tmp_path):
    ...
    assert (restored / "media/L001/L001_1.jpg").read_bytes() == image.read_bytes()
    with pytest.raises(ValueError):
        restore(snapshot, restored)
    with (snapshot / "media/L001/L001_1.jpg").open("ab") as stream:
        stream.write(b"bad")
...
def test_legacy_import_preserves_source_and_is_repeatable(roster, tmp_path):
    ...
    document = {
        ...
        "model": {"status": "active", "price": 20, "source": "Shop"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("M004", "bought", "shipped", None, None),
        ("L143", "missed", "shipped", None, None),
    ],
)
